...
Translating each character of the word <code>'Interconnectedness'</code> to its corresponding numerical value with the Gematria Primus 2013 and summing yields <code>772</code>. The song itself is <code>277.133</code> seconds long.<br>
<br>
...
Download OpenPuff v4.00 (newer won't work). Load `Interconnectedness.mp3`, set `A` to `33011033`, disable `B` and `C`, 
and use the `mp3 > Maximum` setting.

[Output](assets/2014/stage07/magicsquares.txt) (`[CRLF]` is not literal, it means there is a "Windows-style" line break):
